...
const DrawGraph = ({size, graphData, nodes, loading, findNode, setFindNode}) => {
      ...
      <SwitchComponents active={activeComponent}>
        <ForceGraph2D
          ...
          onNodeClick={(node, event) => {
            ...
            socket.emit('row_selected', {data: {node: node.id, name: node.name}, isSelected: !selectedNodes.includes(node.id)});
          }}
        />
        <ForceGraph3D
          name='2D'

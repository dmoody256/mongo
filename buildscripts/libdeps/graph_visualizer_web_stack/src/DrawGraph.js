...
const DrawGraph = ({
  ...
  React.useEffect(() => {
    setPathNodes(
      {fromNode: graphPaths.fromNode, toNode: graphPaths.toNode}
    );
    setPaths(graphPaths.paths);
    var paths = Array();
    for (var path=0; path < graphPaths.paths.length; path++){
      ...
      for (var i=0 ; i<graphPaths.paths[path].length; i++) {
        ...
        pathArr.push({source: graphPaths.paths[path][i-1], target: graphPaths.paths[path][i]});
      }
      paths.push(pathArr);
    }
    setPathEdges(paths);
        ...
        <ForceGraph2D
          ...
          linkDirectionalParticles={d => {
            if (graphPaths.selectedPath >= 0){
              for (var i=0; i < pathEdges[graphPaths.selectedPath].length; i++){
                if (pathEdges[graphPaths.selectedPath][i].source == d.source.id && pathEdges[graphPaths.selectedPath][i].target == d.target.id){
                  return 5;
                }
              }
            }
...
export default connect(getGraphData, { setFindNode, updateCheckbox })(DrawGraph);

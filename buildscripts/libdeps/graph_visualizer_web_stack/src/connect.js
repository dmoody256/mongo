...
export const backend_root = "http://localhost:5000";

export const socket = io.connect(backend_root, {
    reconnection: true,
...
const SocketConnection = ({
  ...
  React.useEffect(() => {
    ...
    fetch(backend_root +'/graph_files').then(res => res.json()).then(data => {
    ...
    socket.on("other_hash_selected", incomingData => {
      selectGraphFile({hash: incomingData.hash, selected: incomingData.selected});
    });

    socket.on("graph_nodes", incomingData => {
        setLoading(false);
        setNodes(incomingData.graphData.nodes.map((node, index) => {
          return { id: index, node: node.id, name: node.name, check: 'checkbox', selected: false };
        }));
    ...
    socket.on("graph_data", incomingData => {
      ...
      if (incomingData.selectedNodes){
        updateCheckboxes(incomingData.selectedNodes.map((node, index) => {

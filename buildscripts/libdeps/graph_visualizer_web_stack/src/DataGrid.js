...
const DataGrid = ({
  ...
  const getRowClassName = ({ index }) => {
    return clsx(index%2==0 ? classes.tableRowEven : classes.tableRowOdd, classes.flexContainer, {
    ...
    if (cellData == 'checkbox'){
        ...
        finalCellData = <Checkbox
          ...
          onChange={event => {
            ...
            socket.emit('row_selected', {data: {node: nodes[rowIndex].node, name: nodes[rowIndex].name}, isSelected: event.target.checked});
          }}
        />;
    }
    else{
  ...
  const headerRenderer = ({ label, columnIndex,  }) => {
    return (
      ...
      >
        <Typography style={{width: '100%' }} align='left' variant="caption" component="h2">
          {label}
        </Typography>
      </TableCell>
    );
...
export default connect(getRows, {updateSelected})(withStyles(styles)(DataGrid));
